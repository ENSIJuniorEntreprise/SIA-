...
// Step 1: Replace Tree Data
const treeRegex = /const divisionTree = \[[\s\S]*?\](?=\n\nexport default function Navbar)/;
if (treeRegex.test(content)) {
    content = content.replace(treeRegex, data);
    console.log("Tree Replaced Successfully");
} else {
    console.error("Failed to match divisionTree");
}

// Step 2: Replace Desktop Render
...
if (content.includes(oldDesktop)) {
    ...
    console.error("Failed to match Desktop Render block");
}

// Step 3: Replace Mobile Render
...
if (content.includes(oldMobile)) {
    ...
    console.error("Failed to match Mobile Render block");
}

fs.writeFileSync('src/components/Navbar.jsx', content, 'utf-8');
console.log("File Write Compelete");
