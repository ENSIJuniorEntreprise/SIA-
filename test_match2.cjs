...
for (const [pageSub, assetSub] of Object.entries(pageDirs)) {
    ...
    for (const jsx of jsxFiles) {
        ...
        if (!match && cleanName === 'etriers') match = 'etriers';
        if (!match && cleanName === 'hydraulique') match = 'hydraulique';
        if (!match && cleanName === 'rotules') match = 'rotules';
        if (!match && cleanName === 'ventillationcopy') match = 'condenseur'; 
        if (!match && cleanName === 'admissionechappement') match = 'AdmissionÉchappement';
        if (!match && cleanName === 'janteroue') match = 'jante roue'; // might not exist
        ...
        if (match && fs.existsSync(path.join(aDir, match))) {
            ...
            console.log(`[OK] ${pageSub}/${jsx} -> ${assetSub}/${match} (${imgs.length} images)`);
        } else {
            console.log(`[--] ${pageSub}/${jsx} -> NO MATCH IN ${assetSub}`);
        }
    }
}
