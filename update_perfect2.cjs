    ...
    categoriesList.forEach((catObj) => {
        ...
        if (matchingFolder) {
            ...
            if (files.length > 0) {
                ...
                const regex = new RegExp(`import\\s+${imageVar}\\s+from\\s+['"][^'"]+['"];?`);
                if (regex.test(content)) {
                     content = content.replace(regex, `import ${imageVar} from "${relativePath}";`);
                     console.log(`Updated ${pageFile}: ${imageVar} -> ${relativePath}`);
                     modifications++;
                } else {
                     console.log(`Regex fail for ${imageVar} in ${pageFile}`);
                }
            }
        }
    });
